...
class hn_captcha
{
	...
	//
		...
		/**
          * @shortdesc Generate only numbers if TRUE
		  * @type boolean
		  * @public
          *
          **/
	...
	//
		...
		function hn_captcha($config,$secure=TRUE)
		{
			...
			if ($this->digitonly) {
				$msg1 = 'Type the <b>'.$this->chars.' chars</b> within <b>0..9</b> below.';
			} else {
	...
	//
		...
		#	$s .= '<form class="captcha" name="captcha1" action="'.$_SERVER['PHP_SELF'].$this->QUERY_STRING.'" method="POST">'."\n";
			...
			$s .= '<p class="captcha_1">
					<input class="captcha" type="text" name="private_key" value="" 
					maxlength="'.$this->chars.'">&nbsp;&nbsp;';
	...
	//
		...
		function make_captcha()
		{
			...
			if(!$res) die('Unable to save captcha-image at '.$this->get_filename());
		}
		...
		{
			if($public=="") $public = $this->public_key;
			$md5 = md5($this->key.$public);
			if ($this->digitonly) {
				$len = strlen($md5);
				$newstring='';
				for ($i=0; $i<=$len; $i++) {
					if (strpos("0123456789",substr($md5, $i, 1))) {
						$newstring .= substr($md5, $i, 1);
					}
				}
				$key = substr($newstring, 0, $this->chars);
			} else {
				$key = substr($md5, 16 - $this->chars / 2, $this->chars);
			}
			return $key;
		}
