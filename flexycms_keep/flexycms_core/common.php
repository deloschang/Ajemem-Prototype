...
//register_shutdown_function("onUnload");
//$error_handle = set_error_handler("onError");
...
//################################################################
//######## TO BE INCUDED IF REQUIRED(Commented on 6th JAN 2009) ##
//################################################################

///include_once(AFIXI_ROOT."configs/".SITE_USED."/permission.php");
///include_once(AFIXI_ROOT."labels/labels.php");
///include_once(AFIXI_ROOT."flexycms_core/classes/ajax/afixi_ajax.class.php");
 // required as on the external server is not picking tihs up as the first file 
...
//$arr1 = parse_ini_file(AFIXI_ROOT."configs/config.ini",true);
$file = file_get_contents(AFIXI_ROOT."configs/".SITE_USED."/config.ini.php");
$file = preg_replace('/\bAPP_ROOT.\b/',APP_ROOT,$file);
//file_put_contents(AFIXI_ROOT."configs/".SITE_USED."/temp.ini.php",$file);
//$arr2 = parse_ini_file(AFIXI_ROOT."configs/".SITE_USED."/temp.ini.php", true);
//$arr2 = parse_ini_file(AFIXI_ROOT."configs/".SITE_USED."/config.ini.php",true);
//$GLOBALS['conf'] = array_merge($arr1,$arr2);
...
if(!get_magic_quotes_gpc()){
	function convertRequest($string){
		global $link;
		if (phpversion() >= '4.3.0'){
			$string = is_array($string) ?array_map('convertRequest', $string) : mysql_real_escape_string($string,$link);//addslashes
		} else {
...
* Provide an array as second argument to get back any error 
* messages recorded
*
*/
...
/*	print_r($stats);
	if($error_handle){
		print "Error occured: ". $error_handle;
	}
}
*/
function execute($sql,&$err)
	{
	   global $stats,$func_name,$link,$report;
	   $time_start = getmicrotime();
	   $AllData = mysql_query($sql);
		if (mysql_errno()){
			$err[0] = mysql_error();
			$err[1] = $sql ;
			$err[2] = mysql_errno() ;
...
function getarray(){
	return mysql_fetch_array(mysql_query("SELECT FOUND_ROWS()"));
}

//function getindexrows($sql, $indexfield, $valuefield=""){
//	$sth = execute($sql,$err);
//	while($row = mysql_fetch_assoc($sth)){
//		$ret[$row[$indexfield]] = $valuefield? $row[$valuefield]: $row;
//	}
//	return ((is_array($ret))? $ret : array());
//}
...
function db_connect(){
		global $link;
		$link = mysql_connect(DB_HOST,DB_USER,DB_PASS,DB_DB) or die("Could not connect: ".mysql_error());
...
function collectinput(&$_input){
	...
	if($_REQUEST['debug_ci']){
		$site_test = LBL_SITE_URL."debug/ci?debug_ci=1";
		echo "You should invoke this url as <a href='$site_test'>$site_test</a><br/>";
		echo "phpversion() : ".phpversion()."<BR>PARSE_FOR_COLLECT_INPUT : ".
			PARSE_FOR_COLLECT_INPUT. "<BR>".'$_SERVER[PARSE_FOR_COLLECT_INPUT] :'. $_SERVER[PARSE_FOR_COLLECT_INPUT];
		$server_var_arr = array('ORIG_PATH_INFO','PATH_INFO','PHP_SELF');
		for($i=0; $i < count($server_var_arr); $i++){
			$a = $server_var_arr[$i];
			echo "<br/>$server_var_arr[$i] : $_SERVER[$a]";
			if($_SERVER[$a] == "/debug/ci"){
				$match_arr[] = $a;
			}
		}
		echo "<br/>You used any of these value : ";
		for($j=0; $j < count($match_arr); $j++){
			$strMatch .= $match_arr[$j]." , ";			
		}
		echo "<i><b>".trim($strMatch," , ")."</b></i> in <b>PARSE_FOR_COLLECT_INPUT</b> constant in <b><i>".$_SERVER['HTTP_HOST'].".constants.php </i></b>file<br/><br/>";
		phpinfo();exit;
	}
...
############################################
######## Return trial pay query string #####
######## using timestamp and sid ###########
############################################
//ex:-  $custom="&ref_page=wall&ref_choice=wall"
function trialpayQstring($custom=''){
	$timestamp=time();
	$sid=$_SESSION['username'];
	$key="fed7ee9f94";//we can add in to config file----Notification key--fed7ee9f94
	$message="timestamp=".$timestamp."&sid=".$sid;
	$hmac=hash_hmac("md5",$message,$key);
	$message="message=".urlencode($message);
	$qstring=$message."&hmac=".$hmac.$custom;
	return $qstring;
}
