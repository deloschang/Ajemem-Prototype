...
#######################################################
	function _listing(){
		$uri = 'flexyadmin/index.php/user/listing';
		$churi = "";
		...
		//,'flag'=>array('STATUS',1,'anchor'=>LBL_ADMIN_SITE_URL."index.php/user/change_status".$churi."/id/",'condition'=>array('0'=>'Enable','1'=>'Disable'))
	...
	function _change_status() {
		...
		if($this->_input['id']) {
			...
			if($err) {
				...
				redirect(LBL_ADMIN_SITE_URL."index.php/user/listing/qstart/".$this->_input['qstart']."/ce/0/".implode('/',$y));
			}
		}
	}
...
#######################################################
	function _update_password() {
		...
		if($rs['password']==$old_pwd) {
			...
			redirect("flexyadmin/index.php/user/change_password");
		}
	}
...
#######################################################
	function _getuserlogin() {
		$uri = 'flexyadmin/index.php/user/getuserlogin/';
...
#######################################################
	function _deleteuserlogin() {
		...
		ob_clean();
		print LBL_ADMIN_SITE_URL."index.php/user/loginusers";
		exit;
	}
...
#######################################################
	function _insert() {
		...
		if($val){
		 ...
		 //$this->_user_listing();
		redirect(LBL_ADMIN_SITE_URL."index.php/user/user_listing");
...
#######################################################
	function _update_user_profile() {
		...
		redirect(LBL_ADMIN_SITE_URL."index.php/user/listing/qstart/".$this->_input['qstart']);
		//}
	}
...
#######################################################
	function _user_listing(){
		$uri = 'flexyadmin/index.php/user/user_listing';
		//$churi = "";
		if($this->_input['email']){
			$cond.=" AND email LIKE '%".$this->_input['email']."%'";
			$uri = $uri."/email/".$this->_input['email'];
...
#######################################################
	function _log_list(){
		$uri = 'flexyadmin/index.php/user/log_list';
		$qstart=isset($this->_input['qstart'])?$this->_input['qstart']:0;
...
##################################################
	function _user_log_details($q='0'){
		...
		$uri = 'flexyadmin/index.php/user/user_log_details/id_user/'.$id_user;		
		$cond=" 1 AND id_user = ".$this->_input['id_user'];
